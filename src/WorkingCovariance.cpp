...
arma::mat WorkingCovariance::compute_covariance(const arma::colvec &time){
  return arma::eye(time.n_elem, time.n_elem);
}

std::vector<arma::mat> WorkingCovariance::compute_covariance(const std::vector<arma::colvec> &time){
  std::vector<arma::mat> out(time.size());
  for(uint i=0; i<time.size(); i++) out[i] = this->compute_covariance(time[i]);
  return out;
}

arma::mat WorkingCovariance::compute_precision(const arma::colvec &time){
  return arma::eye(time.n_elem, time.n_elem);
}
...
void WorkingCovariance::update_parameters(const std::vector<arma::colvec> &r){}

void WorkingCovariance::add_to_results(Rcpp::List& results){}
...
void CompoundSymmetry::update_parameters(
  ...
  for(uint iter=0; iter<20; iter++){
    std::vector<double> d = this->derivatives(sr, Ptmp);
    if(d[1] > 0.){
    ...
    Rcpp::Rcout << "         V." << iter << " pllk=" << pllk << "(re_ratio=" << this->variance_ratio <<
      ", d1=" << d[0] << ", d2=" << d[1] << ")\n";
    if(fabs(pllk - pllk_prev) / fabs(pllk) < 1e-6) return;
    pllk_prev = pllk;
  }
